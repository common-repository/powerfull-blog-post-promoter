...
function bte_opp_tweet($tweet) {
	...
	$snoop = new Snoopy;
	$snoop->agent = 'HOW TO MAKE MONEY ONLINE http://www.blogform.co.cc/';
...
function bte_opp_the_content($content) {
	...
	if (isset($origPubDate) && $origPubDate!='') {
		if ($showPub || $givecredit) {
			...
			if ($givecredit) {
					$dateline.='Republished by  <a href="http://www.blogform.co.cc/wordpress-plugins/powerfull-blog-post-promoter/">Blog Post Promoter</a>';
			}
			$dateline.='</small></p>';
		}
	}
